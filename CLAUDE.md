...
- **Day-cell blend rule** (`heatmapMath.dayPaintPair`): a calendar day's color is a function of its `DaySources` (one dominant plan session + zero or more activity entries). A single source paints with that source's own `{bg, fg}` palette pair unchanged. Two or more sources blend *per variant* via `mixHexColors` — `bg`s mixed with `bg`s, `fg`s with `fg`s — never cross-mixed. Callers pick which variant to render: the heatmap shows `fg` from 2 total entries up (`dayTotalCount(s) >= 2`, matching the existing single-plan-day threshold), `CurrentWeekStrip` always uses `bg` for the cell fill and `fg` for the rail/mark, same as its old plan-only rendering. `CurrentWeekStrip`'s scheduled-day checklist semantics are unchanged by any of this — activities only widen the *unscheduled* branch (calendar view), never touch the ✓/dot logic or the progress bar. The heatmap streak (`currentWeekStreak`) and "last 30 days" count (`daysInLast`) both run over the union of trained-plan-days and activity-days, so an activity-only week keeps a streak alive. `activityColor` (in `planColor.ts`) is a small palette deliberately hue-distant from `planColor`'s palette, both across and within — cyan/pink/teal/deep-purple — so a blended cell still reads as "plan + activity" rather than melting into one plan-like color; keep new palette entries hue-distant too.
